...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
  ...
  empty($address_err) && empty($email_err) && empty($gender_err) && empty($hobby_err) && empty($country_err) && empty($profileimage_err)) {
    ...
    $sql = "UPDATE employees SET first_name = ?, last_name = ?, password = ?,confirm_password = ?,phone = ?,address = ?,
    email = ?, gender = ?, hobby = ?, country = ?, profile_image=? WHERE id = ?";


    if ($stmt = mysqli_prepare($link, $sql)) {

      mysqli_stmt_bind_param($stmt, "ssssissssssi", $param_fname, $param_lname,$param_password,$param_confirmpassword,
      $param_phone,$param_address,$param_email,$param_gender,$param_hobby,$param_country,$param_profile_image,$param_id );
  ...
  <div class="container">
    <div class="row justify-content-center mt-5">
      <div class="col-lg-6">
        ...
        <form action="<?= htmlspecialchars(basename($_SERVER["REQUEST_URI"]));  ?>" class="bg-light p-4 shadow-sm" method="post" enctype="multipart/form-data" novalidate>
          <div class="row gy-3">
            <div class="col-lg-6">
            ...
            <div class="col-lg-6">
                ...
                <input type="checkbox"  name="hobby" value="write" <?= (isset($hobby) && $hobby == "wrie") ? "selected" : ""; ?>>wrie</option>
